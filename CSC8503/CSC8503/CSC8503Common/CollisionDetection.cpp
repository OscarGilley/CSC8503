...
bool CollisionDetection::RayBoxIntersection(const Ray&r, const Vector3& boxPos, const Vector3& boxSize, RayCollision& collision) {
	...
	const float epsilon = 0.0001f; //for float calculation correctness
	for (int i = 0; i < 3; ++i) {
		if (intersection[i] + epsilon < boxMin[i] || intersection[i] - epsilon > boxMax[i]) {
			return false; //best intersection doesn't touch box!
		}
	}
...
bool CollisionDetection::RayCapsuleIntersection(const Ray& r, const Transform& worldTransform, const CapsuleVolume& volume, RayCollision& collision) {
	Vector3 capsulePosition = worldTransform.GetPosition();
	Quaternion orientation = worldTransform.GetOrientation();

	Matrix3 transform = Matrix3(orientation);
	Vector3 upVector = transform * Vector3(0, 1, 0);

	Vector3 topPos = capsulePosition + (upVector * (volume.GetHalfHeight() - volume.GetRadius()));

	Vector3 localPos = r.GetPosition() - capsulePosition;
	...
	Plane p = Plane::PlaneFromTri(capsulePosition, originTop, thirdPoint);

	RayPlaneIntersection(r, p, collision);

	//Debug::DrawLine(collision.collidedAt, r.GetPosition(), Vector4(0, 0, 1, 1), 20.0f);
	//Debug::DrawLine(capsulePosition, topPos, Vector4(0,0,1,1), 5.0f);
	//Debug::DrawLine(capsulePosition, thirdPoint, Vector4(0, 0, 1, 1), 5.0f);
	...
	if (dotDist < 0) {
		Vector3 penetrationDistance = collision.collidedAt - topPos;

		float penetrationFloat = penetrationDistance.Length();

		if (penetrationFloat <= volume.GetRadius()) {
			return true;
		}
	...
	Vector3 bottomPos = capsulePosition - (upVector * (volume.GetHalfHeight() - volume.GetRadius()));
	vA = bottomPos - collision.collidedAt;
	//vA.Normalise();
	...
	if(dotDist < 0) {
		Vector3 penetrationDistance = collision.collidedAt - bottomPos;
	...
	Vector3 cD = collision.collidedAt - capsulePosition;
	//Vector3 extents = Vector3(volume.GetRadius(), volume.GetHalfHeight(), volume.GetRadius());
	float clampedDot = Maths::Clamp(Vector3::Dot(cD, upVector), -volume.GetHalfHeight(), volume.GetHalfHeight());

	Vector3 d = capsulePosition + (upVector * (clampedDot));
	
	float dist = (d - collision.collidedAt).Length();
...
bool CollisionDetection::RaySphereIntersection(const Ray&r, const Transform& worldTransform, const SphereVolume& volume, RayCollision& collision) {
	...
	//Get direction between the ray origin and the sphere origin
...
bool CollisionDetection::OBBSphereIntersection(const OBBVolume& volumeA, const Transform& worldTransformA,
	...
	Vector3 tempTransformA = invTransform * worldTransformA.GetPosition();
	Vector3 tempTransformB = (invTransform * worldTransformB.GetPosition());

	Vector3 boxSize = volumeA.GetHalfDimensions();

	Vector3 delta = tempTransformB - tempTransformA;
	Vector3 closestPointOnBox = Maths::Clamp(delta, -boxSize, boxSize);
	Vector3 localPoint = delta - closestPointOnBox;
	float distance = localPoint.Length();
	...
	//yep we are colliding
	if (distance < volumeB.GetRadius()) {
		//delta = worldTransformB.GetPosition() - worldTransformA.GetPosition();
...
bool CollisionDetection::OBBIntersection(
	...
	Matrix3 boxAOrientation = worldTransformA.GetOrientation();
	Matrix3 boxBOrientation = worldTransformB.GetOrientation();
	...
	if (overlap) {
		static const Vector3 faces[6] =
		{
			boxBOrientation * Vector3(-1, 0, 0), boxAOrientation * Vector3(1, 0, 0),
			boxBOrientation * Vector3(0, -1, 0), boxAOrientation * Vector3(0, 1, 0),
			boxBOrientation * Vector3(0, 0, -1), boxAOrientation * Vector3(0, 0, 1)
		};
...
bool CollisionDetection::SphereCapsuleIntersection(
	...
	//RayPlaneIntersection(, p, collision);
	...
	if (dotDist < 0) {
		...
		if (penetrationFloat <= volumeA.GetRadius() + volumeB.GetRadius()) {
			...
			return SphereIntersection(sphere, transform, volumeB, worldTransformB, collisionInfo);
		}
...
bool CollisionDetection::OBBCapsuleIntersection(const OBBVolume& volumeB, const Transform& worldTransformB,
	const CapsuleVolume& volumeA, const Transform& worldTransformA, CollisionInfo& collisionInfo) {
	...
	Vector3 tempVectorA = invTransform * worldTransformA.GetPosition();
	Vector3 tempVectorB = (invTransform * worldTransformB.GetPosition());

	Transform tempTransformA = Transform();
	tempTransformA.SetPosition(tempVectorA);
	
	Transform tempTransformB = Transform();
	tempTransformB.SetPosition(tempVectorB);

	AABBVolume aabb = AABBVolume(volumeB.GetHalfDimensions());

	bool collided = AABBCapsuleIntersection(aabb, tempTransformB, volumeA, tempTransformA, collisionInfo);
	...
	if (collided) {
		...
		//collisionInfo.point.normal = transform * (collisionInfo.point.normal / 3);
		return true;
	}
