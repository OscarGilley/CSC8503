...
void PhysicsSystem::ImpulseResolveCollision(GameObject& a, GameObject& b, CollisionDetection::ContactPoint& p) const {
	...
	//now to work out the effect of intertia...
	...
	//float cRestitution = 0.5f; //disperse some kinetic energy with e

	float cRestitution = a.GetRestitution() * b.GetRestitution();
	...
	tree.OperateOnContents([&](std::list<QuadTreeEntry<GameObject*>>& data) {
		...
		for (auto i = data.begin(); i != data.end(); i++) {
			for (auto j = std::next(i); j != data.end(); j++) {
				//is this pair of items already in the set,
				//if same pair is in another quadtree node etc
...
void PhysicsSystem::IntegrateAccel(float dt) {
	...
	for (auto i = first; i != last; ++i) {
		...
		Vector3 accel = force * inverseMass; // a = f/m = fm^-1

		if (applyGravity && inverseMass > 0 && object->gravityApplied) {
			accel += gravity; //don't move infinitely heavy things
		}
		
		linearVel += accel * dt; //integrate accel!
		if (object->GetFriction() > 0.9) {
			object->SetLinearVelocity(linearVel * Vector3(object->GetFriction(), 1, object->GetFriction()));
		}
		else {
			object->SetLinearVelocity(linearVel * Vector3(object->GetFriction(), object->GetFriction(), object->GetFriction()));
		}
		...
		angVel += angAccel * dt;
		if (object->GetFriction() > 0.9) {
			object->SetAngularVelocity(angVel * Vector3(object->GetFriction(), 1, object->GetFriction()));
		}
		else {
			object->SetAngularVelocity(angVel * Vector3(object->GetFriction(), 1, object->GetFriction()));
		}
...
void PhysicsSystem::IntegrateVelocity(float dt) {
	...
	//float dampening = first.GetDampening()

	float frameLinearDamping = 1.0f - (0.7f * dt);
	...
	for (auto i = first; i != last; ++i) {
		...
		//damp angular velocity too
		float frameAngularDamping = 1.0f - (0.7f * dt);
